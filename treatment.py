...
class Init:
    ...
    def print(self):
        print(
            str(self.operation_type) + ' t: ' + str(self.t_before) +
            ' real time: ' + str(self.time))

    def table(self, table, deb, ratio):
        table.insert(parent='',
                     index='end',
                     values=(self.operation_type,
                             (self.t_before - deb) / ratio, '', '', '', '', '',
                             '', ''))
        return table
...
class Finalize:
    ...
    def print(self):
        print(
            str(self.operation_type) + ' t: ' + str(self.t_before) +
            ' real time: ' + str(self.time) + ' rank: ' + str(self.rank))

    def table(self, table, deb, ratio):
        table.insert(parent='',
                     index='end',
                     values=(self.operation_type,
                             (self.t_before - deb) / ratio, '', '', self.rank,
                             '', '', '', ''))
        return table
...
class Wait:
    ...
    def print(self):
        print(
            str(self.operation_type) + ' t1: ' + str(self.t_before) + ' t2: ' +
            str(self.t_after) + ' rank: ' + str(self.rank) + ' req: ' +
            str(self.request))
    ...
    def table(self, table, deb, ratio):
        table.insert(parent='',
                     ...
                     values=(self.operation_type,
                             (self.t_before - deb) / ratio,
                             (self.t_after - deb) / ratio, '', self.rank, '',
                             '', '', self.request))
        return table
...
class Irecv:
    ...
    def print(self):
        print(
            str(self.operation_type) + ' t1: ' + str(self.t_before) + ' t2: ' +
            str(self.t_after) + " bytes: " + str(self.nb_bytes) + ' tag: ' +
    ...
    def table(self, table, deb, ratio):
        table.insert(
            parent='',
            index='end',
            values=(self.operation_type, (self.t_before - deb) / ratio,
                    (self.t_after - deb) / ratio, self.nb_bytes, self.rank,
                    self.dest, self.tag, self.comm, self.request))
        return table
    ...
    def draw_timeline(self, deb, canvas, last_op, offset, voffset, ratio):
        ...
        canvas.create_text(
            ...
            str(self.tag) + " on comm " + str(self.comm) + "\n with request " +
            str(self.request),
            anchor=NW)
...
class Recv:
    ...
    def table(self, table, deb, ratio):
        table.insert(parent='',
                     ...
                     values=(self.operation_type,
                             (self.t_before - deb) / ratio,
                             (self.t_after - deb) / ratio, self.nb_bytes,
    ...
    def draw_timeline(self, deb, canvas, last_op, offset, voffset, ratio):
        ...
        canvas.create_text(x0 + 3,
                           y0 + 3,
                           text=str(self.operation_type) + "\n" +
                           str(self.nb_bytes) + " bytes receiv from " +
                           str(self.dest) + "\n" + "with tag " +
                           str(self.tag) + " on comm " + str(self.comm),
                           anchor=NW)
...
class Send:
    ...
    def draw_timeline(self, deb, canvas, last_op, offset, voffset, ratio):
        ...
        canvas.create_text(x0 + 3,
                           y0 + 3,
                           text=str(self.operation_type) + "\n" +
                           str(self.nb_bytes) + " bytes sent to " +
                           str(self.dest) + "\n" + "with tag " +
                           str(self.tag) + " on comm " + str(self.comm),
                           anchor=NW)
...
def make_pair_isw(mpi_operation):
    ...
    for i in range(0, len(mpi_operation)):
        for j in range(i, len(mpi_operation)):
            if ((mpi_operation[i].operation_type == 'Isend') or
                (mpi_operation[i].operation_type
                 == 'Irecv')) and (mpi_operation[j].operation_type == 'Wait'):
...
def make_pair_sr(mpi_operation, ratio):
    ...
    for i in range(0, len(mpi_operation)):
        for j in range(i, len(mpi_operation)):
            if ((mpi_operation[i].operation_type == 'Isend') or
                (mpi_operation[i].operation_type == 'Send')) and (
                    (mpi_operation[j].operation_type == 'Recv') or
                    (mpi_operation[j].operation_type == 'Irecv')):
                if (mpi_operation[i].tag == mpi_operation[j].tag) and (
                        mpi_operation[i].dest == mpi_operation[j].rank
                ) and (mpi_operation[i].rank == mpi_operation[j].dest) and (
                        mpi_operation[i].comm == mpi_operation[j].comm
                ) and (mpi_operation[i].nb_bytes == mpi_operation[j].nb_bytes):
                    pair_sr.append(
                        pair_send_receiv(mpi_operation[i], mpi_operation[j],
                                         ratio))
                    break
            if ((mpi_operation[j].operation_type == 'Isend') or
                (mpi_operation[j].operation_type == 'Send')) and (
                    (mpi_operation[i].operation_type == 'Recv') or
                    (mpi_operation[i].operation_type == 'Irecv')):
                if (mpi_operation[j].tag == mpi_operation[i].tag) and (
                        mpi_operation[j].dest == mpi_operation[i].rank
                ) and (mpi_operation[j].rank == mpi_operation[i].dest) and (
                        mpi_operation[j].comm == mpi_operation[i].comm
                ) and (mpi_operation[j].nb_bytes == mpi_operation[i].nb_bytes):
                    pair_sr.append(
                        pair_send_receiv(mpi_operation[j], mpi_operation[i],
                                         ratio))
                    break
    return pair_sr
...
def gather_info(liste_mpi_op, liste_isw):
    ...
    for elem in liste_isw:
        ...
        mpi_cost = mpi_cost + (elem.op1.t_after - elem.op1.t_before +
                               elem.op2.t_after - elem.op2.t_before)
